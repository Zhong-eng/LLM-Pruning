...
if __name__ == "__main__":
    ...
    with torch.profiler.profile(
        ...
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/layer_prune'),
        record_shapes=True,
        profile_memory=True,
        with_stack=True
    ) as prof:
        for j in range(50):
            ...
            # random select some attention layers to use for this epoch
            random_index = sorted(random.sample(range(12), NUM_ATTENTION_KEPT))
            record =  model.bert_model.encoder.layer
            select_layer = nn.ModuleList()
            for i in range(NUM_ATTENTION_KEPT):
                select_layer.append(record[random_index[i]])
            model.bert_model.encoder.layer = select_layer
            ...
            for batch, dl in enumerate(train_loader):
                ...
                optimizer.step()
            end_time = time.time()
            print(f"{end_time - start_time} seconds for this epoch")
            model.eval()
            num_correct = 0
            num_samples = 0
